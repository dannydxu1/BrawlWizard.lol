...
for link in brawler_links:
    ...
    if brawler_soup:
        print(link)
        floatleft_divs = brawler_soup.find_all('div', class_='floatleft')
        for div in floatleft_divs:  
            img_tag = div.find('img', {'data-image-name': lambda x: x and 'Portrait' in x})
...
for img_url in cleaned_image_urls:
    ...
    file_path = os.path.join('portraits', img_name)
